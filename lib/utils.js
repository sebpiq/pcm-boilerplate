...
var StreamDecoder = module.exports.StreamDecoder = function(format, opts) {
  ...
  this._sizeRatio = Math.round(this.format.bitDepth / 8) * this.format.numberOfChannels
...
_.extend(StreamDecoder.prototype, Transform.prototype, {
  ...
  read: function(size) {
    if (size !== undefined) size *= this._sizeRatio
    var buffer = Transform.prototype.read.call(this, size)
    if (buffer && buffer.length < size && this.opts.pad) {
      buffer = Buffer.concat([buffer, new Buffer(size - buffer.length)])
    }
    return buffer ? this._decoder(buffer) : buffer
  },
